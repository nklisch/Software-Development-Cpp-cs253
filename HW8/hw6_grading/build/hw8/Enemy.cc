...
{
name = rhs.name;
myGallery = rhs.myGallery;
validKeys = rhs.validKeys;
keyValues = rhs.keyValues;
showName = rhs.showName;
showOther = rhs.showOther;
showLink = rhs.showLink;
otherKeys = rhs.otherKeys;
linkKeys = rhs.linkKeys;
maxLinksLength = rhs.maxLinksLength;
maxOthersLength = rhs.maxOthersLength;
return *this;
...
{
    if(empty())
        throw range_error("Key " + key + " not found, enemy is empty");
  if(key == "Name")
    return name;
