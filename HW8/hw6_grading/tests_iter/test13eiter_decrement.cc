...
void test_enemy_iterators_decrement(const Gallery &g) {
    ...
    int index = find_by_name(g,"Galadriel");
...
int main() {
   try{
      ...
      test_enemy_iterators_decrement(g);
        ...
        cerr << "Test 13: Unexpected exception: " << e.what() <<" --failed"<< '\n';
        return 1;
   }
   catch (...) {
        cerr << "Test 13: Caught something, but it’s not a std::exception‽"<< "--failed"<< '\n';
        return 2;
   }
   cout<<"test13: Enemy iterator decrement -- passed"<<'\n';
   return 0;
}
