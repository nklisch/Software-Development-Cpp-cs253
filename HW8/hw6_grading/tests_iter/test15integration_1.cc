...
int main() {
    try {
        const Gallery vacant("lorKeys", "/dev/null");
        ...
        test_comparison_operators(roles);
        test_gallery_iterators(roles);
        test_enemy_iterators(roles);

        show(vacant);
        show(roles);
    }
    catch (const exception &e) {
        cerr << "Test 15: Unexpected exception: " << e.what() << '\n';
        return 1;
    }
    catch (...) {
        cerr << "Test 15: Caught something, but it’s not a std::exception‽\n";
        return 2;
    }
